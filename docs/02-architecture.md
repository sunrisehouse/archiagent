...
## 5. 도구 구현 — 순수 함수 · 미리보기 게이트

- **목차 제목:** 도구 구현
- **거버닝 메시지:** 도구는 순수 함수다. 모델은 추출·생성에만 쓰고, 조회·저장·출력은 결정적으로 짠다. 저장은 미리보기 → 확인 → 반영으로 나눈다.
- **내부 컨텐츠:**
  - **순수 함수** — 각 도구는 `(store[, model], 입력) → 결과` 형태. 부수효과를 최소화해 단위 테스트가 쉽다.
  - **모델은 추출·생성만** — `kb_artifact_ingest`·`kb_doc_generate`만 모델을 부른다. `kb_graph_query`(영향도·추적성)·`kb_doc_export`는 저장된 사실에서 모델 없이 계산한다.
  - **구조화 출력** — 모델 응답을 JSON 스키마로 받고 파싱·검증한다(산문 파싱 금지). 코드펜스·서론이 섞여도 첫 JSON 객체만 안전하게 추출한다.
